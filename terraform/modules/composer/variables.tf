...
variable "airflow_config_overrides" {
  type = object({
    core-load_example = string
    celery-worker_concurrency=number
    core-non_pooled_task_slot_count=number
    core-dag_concurrency =number
    core-dagbag_import_timeout=number
    core-parallelism=number
    scheduler-max_threads=number
  })

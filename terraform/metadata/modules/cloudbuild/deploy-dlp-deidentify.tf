...
resource "google_cloudbuild_trigger" "deploy-dlp-deidentify-trigger" {
  project = var.gcp_project_id
  name = "deploy-dlp-deidentify"
  description = "Import DLP deidentify templates from GIT."
  substitutions = {
    _MODE = "SYNC"
  }
  source_to_build {
    ...
    step {
      ...
      args = [
                      ...
                      <<-EOT
                        ...
                        python3 ./import_dlp-deidentify.py -t $BRANCH_NAME -m $_MODE -c $$CFG -f /workspace/$$GRIZZLY_REPO
                      EOT
      ]
    }
  }

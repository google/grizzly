...
resource "google_cloudbuild_trigger" "deploy-datacatalog-trigger" {
  project = var.gcp_project_id
  name = "deploy-datacatalog"
  description = "Deploy DataCatalog templates from GIT."
  substitutions = {
    _MODE = "SYNC"
  }
  source_to_build {
    ...
    step {
      ...
      args = [
                      ...
                      <<-EOT
                        ...
                        echo "git checkout -b $BRANCH_NAME --track remotes/origin/dev" &&
                        ...
                        python3 ./import_data-catalog.py -t $BRANCH_NAME -m $_MODE -c $$CFG -f /workspace/$$GRIZZLY_REPO
                      EOT
      ]
    }
  }

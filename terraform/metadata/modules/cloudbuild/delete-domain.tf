...
resource "google_cloudbuild_trigger" "delete-domain-trigger" {
  project = var.gcp_project_id
  name = "delete-domain"
  description = "Remove a deployed domain from Composer"
  substitutions = {
    _DOMAIN = "YOUR_DOMAIN"
    _ENVIRONMENT = "dev"
  }
    ...
    step {
      ...
      args = [
                      ...
                      <<-EOT
                        ...
                        cd /workspace/$$GRIZZLY_FRAMEWORK_REPO/scripts &&
                        python3 ./delete_domain.py -p "$$${GCP_ENVIRONMENT}" -l "$$${AIRFLOW_LOCATION}" -c "$$${AIRFLOW_ENVIRONMENT}" -d $_DOMAIN -bid "$BUILD_ID" -x "$COMMIT_SHA" &&

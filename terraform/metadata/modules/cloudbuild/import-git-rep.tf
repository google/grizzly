...
resource "google_cloudbuild_trigger" "import-git-rep-trigger" {
  project = var.gcp_project_id
  name = "import-git-rep"
  description = "Import Git repository objects into BQ Git Metadata"
  substitutions = {
  }
  trigger_template {
    branch_name = ".*"
    ...
    step {
      ...
      args = [
                      ...
                      <<-EOT
                          ...
                          git remote -v &&  git branch -a &&  pip3 install -r /workspace/$$GRIZZLY_FRAMEWORK_REPO/requirements.txt &&
                          cd /workspace/$$GRIZZLY_FRAMEWORK_REPO/scripts &&
                          python3 ./import_git.py -e $BRANCH_NAME -c $$CFG -f /workspace/$$GRIZZLY_REPO
                        EOT
      ]
    }
  }

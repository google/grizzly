...
def create_job_build_tmp_table(bq_utils: BQUtils) -> Table:
  """Create temporary table with data for job_build table.

  Args:
    bq_utils (BQUtils): Contains instance of BQ Client and methods for work
      with BQ tables.

  Returns:
    (Table): Instance of BQ table.
  """
...
def create_job_build_table(bq_utils: BQUtils) -> Table:
  """Create permanent BQ job_build table.
...
def merge_job_build_tmp_to_job_build(bq_utils: BQUtils,
  ...
  """Merge data between temporary and permanent job_build table.

  Perform BQ MERGE statement for merge data into target permanent job_build
  table.

  Args:
    bq_utils (BQUtils): Contains instance of BQ Client and methods for work
      with BQ tables.
    job_build_tmp_table_name (str): Name of job_build temporary table.
  """
...
def get_subject_area_build_files_rows(bq_client: Any,
                                      commit_sha: str) -> List[Dict[str, Any]]:
  """Return rows from the subject_area_build_files table.

  Args:
    bq_client (Any): Instance of BQ Client
    commit_sha (str): Commit SHA used by the GCP build.

  Returns:
    (List[Dict[str, Any]]): List of files to be included into deployment.
  """
...
def scope_to_job_build_rows(
    scope: Scope,
    commit_sha: str,
    build_files: List[Dict[str, Any]]) -> List[Dict[str, str]]:
  """Return scope data to insert into job_build table.

  Generate a list of dictionaries with definition of BQ table rows.

  Args:
    scope (Scope): Instance of Scope that contains ETL definition for DAG
      with all yml, sql, json, etc. files to be deployed on GCP Composer
      environment.
    commit_sha (str): Commit SHA used by the GCP build.
    build_files (List[Dict[str, Any]]): List of files to be included into
      deployment. This list is generated by get_subject_area_build_files_rows
      function.

  Returns:
    List[Dict[str, str]]: List of rows to be inserted into job_build table.
  """
...
def main(args: argparse.Namespace):
  ...
  job_build_tmp_table = create_job_build_tmp_table(bq_utils=bq_utils)
  create_job_build_table(bq_utils=bq_utils)

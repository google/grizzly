...
"""Import DLP inspection templates.

  Script used by Cloud Build to import DLP inspection templates to a project.
  Script used for SYNC/MERGE of DLP templates.

  Typical usage example:

  python3 ./import_dlp-inspect.py -t $BRANCH_NAME -m $_MODE
...
class ImportToolConfig:
  ...
  Attributes:
    ...
    gcp_environment_source (str): environment configuration from
...
def delete_templates(config: ImportToolConfig,
                     templates: Dict[str, InspectTemplate]) -> None:
  """Deletes set of DLP Inspection templates from project.

  Args:
    config (ImportToolConfig): ImportToolConfig object containing configuration
      data.
    templates (Dict[str, InspectTemplate]): A dictionary of templates to delete.
  ...
  for template_id, _ in templates.items():
    try:
      del_request = dlp.DeleteInspectTemplateRequest()
      del_request.name = config.target_template_id.format(
          template_id=template_id)
      dlp_client.delete_inspect_template(request=del_request)
...
def create_templates(config: ImportToolConfig,
                     templates: Dict[str, InspectTemplate]) -> None:
  """Creates set of DLP Inspection templates for project.

  Args:
    config (ImportToolConfig): ImportToolConfig object containing configuration
      data.
    templates (Dict[str, InspectTemplate]): A dictionary of templates to create.
      Keys are template names. Values are templates.
  ...
  for template_id, template in templates.items():
    target_template = dlp.CreateInspectTemplateRequest()
    target_template.inspect_template = template
    target_template.template_id = template_id
    target_template.parent = config.parent_target
    dlp_client.create_inspect_template(request=target_template)


def sync_templates(config: ImportToolConfig,
                   source_templates: Dict[str, InspectTemplate],
                   target_templates: Dict[str, InspectTemplate]) -> None:
  """Syncs the DLP Inspection templates between source and target.

  Args:
    config (ImportToolConfig): ImportToolConfig object containing configuration
      data.
    source_templates (Dict[str, InspectTemplate]): Dict with keys of template
      names and values of definitions.
    target_templates (Dict[str, InspectTemplate]): Dict with keys of template
      names and values of definitions.
  ...
  delete_templates(config=config,
                   templates=target_templates_deleting)
  # create template in target
  create_templates(config=config,
                   templates=source_templates)


def load_source_proto(folder: str) -> Dict[str, InspectTemplate]:
  ...
  Returns:
    (Dict[str, InspectTemplate]): A dictionary with keys of tag names and values
      of tag definitions.
  """
  path = f'/{folder}/{FILES_FOLDER}/'
  templates = DeploymentUtils.proto_load(
      path, '*.gcp_proto', proto_class=InspectTemplate)
...
def main(args: argparse.Namespace) -> None:
  ...
  Imports DLP Inspection templates to a project. Script used for SYNC/MERGE of
  DLP templates.
  ...
  target_dic = get_dic_inspection_templates(config.parent_target)
